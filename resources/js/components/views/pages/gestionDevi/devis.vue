...
const telechagerDevi = async (id) => {
    await axios
        .post("/api/download_devi", { id: id }, { responseType: "arraybuffer" })
        .then((response) => {
...
const deleteDevi = (deviId) => {
    Swal.fire({
        ...
        if (result.value) {
            axios.get("/api/delete_devi/" + deviId).then(() => {
                console.log("success", deviId);

                Swal.fire("Delete", "devis delete successfully", "success");
                ...
                <div class="col-span-12">
                    <div class="card" style="background-color: #e1e4f4">
                        <div class="card-header pb-4">
                            <div
                                class="flex gap-2 justify-between items-center 3sm:flex-col 3sm:items-end"
                            >
                                <div class="flex items-center m-0">
                                    <input
                                        class="py-[11px] px-[25px] dark:bg-sidebar-dark2 dark:border-border-dark dark:text-white rtl:rounded-bl-none rtl:rounded-tl-none border rounded w-full text-xs text-Gray-darker rounded-tr-none rounded-br-none"
                                        type="Search"
                                        placeholder="Search"
                                        v-model="searchQuery"
                                    ...
                                    />
                                    <button
                                        class="bg-primary text-white rounded border-none p-3 rounded-tl-none rounded-bl-none rtl:rounded-bl-5 rtl:rounded-br-none rtl:rounded-tr-none rtl:rounded-tl-5"
                                    >
                                        <svg
                                            class="w-[20px] h-[20px] stroke-white"
                                        >
                                            <use
                                                href="https://admin.pixelstrap.com/govo/assets/svg/sprite.svg#Search"
                                            ></use>
                                        </svg>
                                    </button>
                                </div>
                        ...
                        <div class="card-body">
                            <div class="overflow-auto custom-scroll">
                                ...
                                >
                                    ...
                                    <tbody>
                                        <tr
                                            class="hover:bg-warning dark:border-border-dark group hover:text-white"
                                            v-for="devi in devis"
                                            :key="devi.id"
                                        >
                                                ...
                                                >
                                                    ...
                                                    >
                                                        <div>
                                                            ...
                                                            >
                                                                <h4
                                                                    class="text-xs font-semibold text-dark"
                                                                >
                                                                    {{
                                                                        devi.id
                                                                    }}
                        ...
                        <div class="card-footer">
                            <div
                                class="flex justify-between items-center flex-wrap sm:flex-col sm:items-end"
                            >
                                <div class="flex items-center">
                                    <button
                                        ...
                                        @click="
                                            getdevis(pagination.currentPage - 1)
                                        "
                                    >
                                        <svg
                                            class="w-4 h-4 stroke-title group-hover:stroke-white"
                                        ></svg>
